...
export default async function handler(
  ...
  try {
    ...
    console.log(`Starting language analysis for evaluation ${evaluationId}`);
    ...
    // Get language skills from the database
    const { data: languageSkills, error: skillsError } = await supabase
      .from('skills')
      .select('*')
      .eq('category', 'language');
    
    if (skillsError || !languageSkills) {
      console.error('Error fetching language skills:', skillsError);
      return res.status(500).json({ message: 'Failed to fetch language skills' });
    }
    
    // Analyze the transcript using Google Gemini
    const model = genAI.getGenerativeModel({ model: "gemini-pro" });
    
    // Build the prompt with audience information
    const prompt = `
      ...
      Here are the skills to evaluate (format: ID. Skill Name - Description):
      ${languageSkills.map((skill: any) => 
        `${skill.id}. ${skill.name} - ${skill.description || 'No description'}`
      ).join('\n')}
    ...
    for (const [skillId, data] of Object.entries(analysis) as [string, any][]) {
      ...
      if (skill) {
        structuredAnalysis[skill.id.toString()] = {
          ...
          isGoodSkill: skill.is_good_skill
        };
      }
    }
    ...
    return res.status(500).json({ 
      message: 'Failed to analyze language',
