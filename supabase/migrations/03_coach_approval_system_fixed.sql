...
-- Create view for pending coach approvals - Including security in the view definition
...
WHERE
  cp.status = 'pending'
  -- Include admin check directly in the view
  AND EXISTS (
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Policy didn't exist or other error, continue
  END;
...
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'Error creating policy on notifications: %', SQLERRM;
END
$$;
...
-- Add the trigger to the coach_profiles table (if it doesn't exist)
DO $$
BEGIN
  -- Drop trigger if it exists
  BEGIN
    DROP TRIGGER IF EXISTS coach_status_change_notification ON public.coach_profiles;
  EXCEPTION WHEN OTHERS THEN
    -- Trigger didn't exist or other error, continue
  END;
...
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'Error creating trigger on coach_profiles: %', SQLERRM;
END
$$; 